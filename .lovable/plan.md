...
- **4c** — Implement member-side writes against `member_set_logs` / `member_workout_completions` / `member_exercise_notes` for the three new raw upsert methods. Teach the offline queue handler to route through the active adapter (or split into per-adapter queue keys). Replace the 657-line member route with a ~80-line shim that wires auth/heartbeat/offline queue around `<WorkoutDayView adapter={memberAdapter} navigation={memberNav} />`. Capability-gate impersonation UI. Playwright smoke.
