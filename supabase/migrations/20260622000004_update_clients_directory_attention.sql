-- ============================================================
-- Update admin_clients_directory RPC
-- Adds: missed workouts detection, inactive client flag,
--       days_inactive, missed_workouts_count, last_active_at
-- Fixes: activity sort now uses last_active_at (not updated_at)
-- Adds: f_missed_workouts and f_inactive to attention scoring
-- ============================================================
...
BEGIN
  ...
  -- Missed workouts: scheduled days in the past 14 days that have no completion record
  missed_workouts AS (
    SELECT
      d.client_id,
    ...
    WHERE b.client_id IN (SELECT id FROM base)
      ...
      -- Only count days that have exercises (not rest days)
      AND EXISTS (SELECT 1 FROM public.pl_rows r WHERE r.day_id = d.id LIMIT 1)
      -- Not completed
      AND NOT EXISTS (
  ...
  enriched AS (
    SELECT
      ...
      -- next block (upcoming, not yet started)
      ...
      -- Missed workouts data
      COALESCE(mw.missed_count, 0) AS missed_workouts_count,
      -- Days inactive (using last_active_at, fallback to last_login_at, fallback to created_at)
      CASE
      ...
      -- NEW: missed workouts flag (2+ missed in last 14 days)
      (COALESCE(mw.missed_count, 0) >= 2) AS f_missed_workouts,
      -- NEW: inactive flag (no activity in 7+ days, client has an active program)
      (
        cb.id IS NOT NULL
  ...
  scored AS (
    SELECT e.*,
      CASE
        ...
        -- NEW: missed workouts is high priority (between review and missing program)
        ...
        -- NEW: inactive client with active program
    ...
    SELECT *,
      row_number() OVER (
        ORDER BY
          ...
          -- FIXED: activity sort now uses last_active_at (was updated_at)
