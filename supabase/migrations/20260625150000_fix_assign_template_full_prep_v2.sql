-- Fix pl_assign_template_to_client to fall back to payload->'blocks' for full_prep
-- templates that use schema v2 (where blocks_data is absent). This caused
-- newly-assigned preps to be created with zero blocks/weeks/rows.
...
BEGIN
  ...
  IF v_type = 'full_prep' THEN
    ...
    v_blk_idx := 0;
    v_cursor_date := p_start_date;
    FOR v_b IN SELECT * FROM jsonb_array_elements(COALESCE(NULLIF(v_payload->'blocks_data','[]'::jsonb), NULLIF(v_payload->'blocks','[]'::jsonb), '[]'::jsonb)) LOOP
