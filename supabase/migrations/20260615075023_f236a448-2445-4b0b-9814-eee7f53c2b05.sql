...
BEGIN
  ...
  filtered AS (
    ...
    WHERE p_status IS NULL OR p_status = '' OR p_status = 'all'
      OR (p_status = 'needs_setup' AND cs.f_needs_setup)
  ...
  SELECT
    COALESCE((SELECT jsonb_agg(to_jsonb(o.*) - 'rn' ORDER BY o.rn) FROM ordered o WHERE o.rn > p_offset AND o.rn <= p_offset + p_limit), '[]'::jsonb),
