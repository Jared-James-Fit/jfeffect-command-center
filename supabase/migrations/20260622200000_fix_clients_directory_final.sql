-- Final fix: replace admin_clients_directory with working version (no broken missed_workouts CTE)
...
BEGIN
  ...
  enriched AS (
    SELECT
      ...
      -- required agreement unsigned, OR
      -- no preferred training days configured yet (a real onboarding step).
      (
      ...
      -- NEW CLIENT: joined within 7 days AND not yet fully active.
      ...
      -- helper: client has an active program right now
      (cb.id IS NOT NULL AND (cb.end_date IS NULL OR cb.end_date >= v_today)) AS f_has_active_program,
      -- helper: account considered activated for primary-action gating
  ...
  scored AS (
    SELECT e.*,
      CASE
        ...
        -- Only treat Needs Setup as a top-priority primary action when the
        -- account is actually NOT activated yet. Setup gaps on already-active
        -- clients (e.g. preferred days) fall through to lower priority.
