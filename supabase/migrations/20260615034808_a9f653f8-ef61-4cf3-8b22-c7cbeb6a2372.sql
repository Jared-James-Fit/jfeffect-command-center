  ...
  IF v_type = 'block'
    ...
    IF jsonb_array_length(v_assignable) > 1 THEN
      ...
      FOR v_b IN SELECT * FROM jsonb_array_elements(v_assignable) LOOP
        -- v2 canonical blocks store weeks under `weeks`; legacy mirrors use `weeks_data`.
    ...
    ELSE
      -- Single assignable block: fall through to the standalone path with weeks_data populated.
      v_payload := jsonb_set(
        v_payload, '{weeks_data}',
        COALESCE(v_assignable->0->'weeks', v_assignable->0->'weeks_data', '[]'::jsonb),
        true
      );
      -- Carry the block name forward so the assigned pl_blocks row matches what the coach saw.
      IF COALESCE(NULLIF(v_assignable->0->>'name',''), '') <> '' THEN
        v_name := v_assignable->0->>'name';
      END IF;
    END IF;
  END IF;
  ...
  IF v_mode IN ('into_block','into_week','into_day') THEN
    ...
    IF v_type = 'exercise_row' THEN
      ...
      INSERT INTO pl_exercise_rows (
        ...
        NULLIF(v_payload->>'rest_seconds','')::int,
