...
BEGIN
  ...
  cur_block AS (
    -- Prefer a block that is actively running today, then the most recently
    -- started block whose end date is still recent. This keeps the "current"
    -- block stable even when a future block has been pre-assigned.
    ...
    WHERE b.archived = false
      ...
      AND (b.start_date IS NULL OR b.start_date <= v_today)
    ORDER BY b.client_id,
      -- active (today between start and end) ranks first
      (CASE
        WHEN b.start_date IS NOT NULL AND b.start_date <= v_today
             AND (b.end_date IS NULL OR b.end_date >= v_today) THEN 0
        ELSE 1
      END),
  ...
  next_block AS (
    -- The soonest upcoming block (starts after today, or after the current
    -- block's end date). Used to surface what's next on the client card.
    SELECT DISTINCT ON (b.client_id)
      b.client_id, b.id, b.name, b.start_date, b.end_date, b.status
    FROM public.pl_blocks b
    LEFT JOIN cur_block cb ON cb.client_id = b.client_id
    WHERE b.archived = false
      AND b.client_id IN (SELECT id FROM base)
      AND b.start_date IS NOT NULL
      AND b.start_date > v_today
      AND (cb.id IS NULL OR b.id <> cb.id)
    ORDER BY b.client_id, b.start_date ASC
  ),
  cur_nut AS (
