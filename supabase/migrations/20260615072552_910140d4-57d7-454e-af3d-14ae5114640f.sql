-- Clients directory RPC: paginated rows + summary counts in one round-trip,
-- scoped by caller role (admin = all, coach = assigned only).

CREATE INDEX IF NOT EXISTS idx_clients_archived_updated ON public.clients (archived, updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_clients_assigned_coach ON public.clients (assigned_coach_id) WHERE archived = false;
CREATE INDEX IF NOT EXISTS idx_clients_status ON public.clients (status) WHERE archived = false;
CREATE INDEX IF NOT EXISTS idx_pl_blocks_client_active ON public.pl_blocks (client_id, end_date) WHERE archived = false;

CREATE OR REPLACE FUNCTION public.admin_clients_directory(
  p_search text DEFAULT NULL,
  p_status text DEFAULT NULL,          -- 'all'|'needs_setup'|'needs_review'|'program_ending'|'payment_issues'|'new_clients'
  p_coaching_type text DEFAULT NULL,
  p_coach_id uuid DEFAULT NULL,
  p_sort text DEFAULT 'attention',     -- 'attention'|'recent'|'name'|'ending'|'activity'
...
BEGIN
  ...
  -- Latest non-archived active or upcoming block per client
  cur_block AS (
  ...
  -- Latest nutrition/cardio target per client
  cur_nut AS (
  ...
  -- Pending review count per client
  pending_reviews AS (
  ...
  enriched AS (
    SELECT
      ...
      -- Derived flags
      (b.account_status IS DISTINCT FROM 'Account Created'
        AND b.account_status IS DISTINCT FROM 'Active') AS f_needs_setup,
      ...
      (b.created_at > now() - interval '14 days') AS f_new_client,
  ...
  scored AS (
    SELECT e.*,
      ...
      CASE
        ...
        WHEN f_needs_setup THEN jsonb_build_object('kind','setup','label','Complete Setup')
        WHEN f_needs_review THEN jsonb_build_object('kind','review','label','Review Check-In')
        WHEN f_missing_program THEN jsonb_build_object('kind','assign','label','Assign Program')
  ...
  filtered AS (
    ...
    WHERE (p_search IS NULL OR p_search = '' OR
      ...
      AND (
        ...
        OR (p_status = 'needs_setup' AND s.f_needs_setup)
        OR (p_status = 'needs_review' AND s.f_needs_review)
        OR (p_status = 'program_ending' AND s.f_program_ending)
        OR (p_status = 'payment_issues' AND s.f_payment_issue)
        OR (p_status = 'new_clients' AND s.f_new_client)
      )
  ),
  ordered AS (
    SELECT *,
  ...
  SELECT
    COALESCE(jsonb_agg(to_jsonb(o.*) - 'rn' ORDER BY o.rn) FILTER (WHERE o.rn > p_offset AND o.rn <= p_offset + p_limit), '[]'::jsonb),
    COUNT(*)
  INTO v_rows, v_total
  FROM ordered o;

  -- Summary counts over the role-scoped, but NOT status-filtered, set
  SELECT jsonb_build_object(
    'all', COUNT(*),
    'needs_setup', COUNT(*) FILTER (WHERE f_needs_setup),
