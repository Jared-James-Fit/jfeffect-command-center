...
BEGIN
  ...
  -- v2 "block" templates: blocks can live under `blocks` (canonical)
  -- OR under `blocks_data` (legacy / partially-migrated payload). The
  -- editor's normalizer treats both as valid, so the assignment RPC
  -- must too — otherwise the client's workout library stays empty.
  IF v_type = 'block'
    ...
    -- single-block v2 path: copy that one block directly
    v_b := v_assignable->0;
    v_weeks_data := COALESCE(v_b->'weeks', v_b->'weeks_data', '[]'::jsonb);
    INSERT INTO pl_blocks (client_id, prep_id, name, weeks, training_focus, status, client_visible, source_template_id, start_date, end_date)
    VALUES (
