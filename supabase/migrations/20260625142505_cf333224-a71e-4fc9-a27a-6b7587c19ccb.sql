...
BEGIN
  ...
  IF v_type = 'block'
    ...
    IF jsonb_array_length(v_assignable) > 1 THEN
      ...
      FOR v_b IN SELECT * FROM jsonb_array_elements(v_assignable) LOOP
        ...
        v_blk_weeks := GREATEST(jsonb_array_length(v_weeks_data), 1);
        v_blk_start := v_cursor_date;
        v_blk_end := CASE WHEN v_cursor_date IS NULL THEN NULL
                          ELSE v_cursor_date + ((v_blk_weeks * 7) - 1) END;
        v_blk_status := CASE WHEN v_blk_idx = 0 THEN 'Active' ELSE 'Draft' END;

        INSERT INTO pl_blocks (client_id, prep_id, name, weeks, training_focus, status, client_visible, source_template_id, start_date, end_date, sort_order)
        VALUES (
          p_client_id, v_prep_id, COALESCE(NULLIF(v_b->>'name',''), 'Block'),
          v_blk_weeks,
          NULLIF(v_b->>'training_focus',''),
          v_blk_status, v_visible, v_tpl.id, v_blk_start, v_blk_end, v_blk_idx
  ...
  IF v_type = 'full_prep' THEN
    ...
    v_blk_idx := 0;
    v_cursor_date := p_start_date;
    -- FIX: fall back to payload->'blocks' (schema v2 location) when
    -- blocks_data is missing/empty. Without this fallback, full_prep
    -- templates saved in v2 format produced empty preps with zero blocks.
    FOR v_b IN SELECT * FROM jsonb_array_elements(
      COALESCE(
        NULLIF(v_payload->'blocks_data','[]'::jsonb),
        NULLIF(v_payload->'blocks','[]'::jsonb),
        '[]'::jsonb
      )
    ) LOOP
      v_weeks_data := COALESCE(v_b->'weeks_data', v_b->'weeks', '[]'::jsonb);
      v_blk_weeks := GREATEST(COALESCE(jsonb_array_length(v_weeks_data), COALESCE((v_b->>'weeks')::int, 4)), 1);
      v_blk_start := v_cursor_date;
      v_blk_end := CASE WHEN v_cursor_date IS NULL THEN NULL
                        ELSE v_cursor_date + ((v_blk_weeks * 7) - 1) END;
      v_blk_status := CASE WHEN v_blk_idx = 0 THEN 'Active' ELSE 'Draft' END;
  ...
  RAISE EXCEPTION 'Unsupported template type or placement mode';
END;
$function$;

-- Cleanup: remove the empty assignment created for Yannick Ring by the buggy
-- version of this function. With the fix in place, an admin can re-run the
-- "Assign" action on the template and it will materialize properly.
DELETE FROM public.pl_preps
WHERE id = 'ed464329-3f11-4f52-95b5-79acf353de54'
  AND NOT EXISTS (
    SELECT 1 FROM public.pl_blocks WHERE prep_id = 'ed464329-3f11-4f52-95b5-79acf353de54'
  );