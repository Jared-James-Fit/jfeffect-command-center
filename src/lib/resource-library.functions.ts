...
async function assertCoachOrAdmin(ctx: any) {
  const { supabase, userId } = ctx;
  const { data, error } = await supabase.rpc("is_coach_or_admin", { _user_id: userId });
  if (error) throw new Error(error.message);
  if (!data) throw new Error("Admins and coaches only");
}
...
export const listFolders = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await context.supabase
      .from("resource_folders").select("*").order("sort_order").order("name");
    if (error) throw new Error(error.message);
    return { folders: data ?? [] };
  });
...
export const updateFolder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertCoachOrAdmin(context);
    const { id, ...patch } = data;
    const { error } = await context.supabase.from("resource_folders").update(patch).eq("id", id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const deleteFolder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertCoachOrAdmin(context);
    const { error } = await context.supabase.from("resource_folders").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const getSignedReadUrl = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: sig, error } = await supabaseAdmin.storage
      .from("resource-library").createSignedUrl(data.path, 3600);
...
export const getUploadUrl = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: signed, error } = await supabaseAdmin.storage
      .from("resource-library").createSignedUploadUrl(path);
...
export const updateResource = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await context.supabase.from("resources").update(patch).eq("id", id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const deleteResource = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row } = await context.supabase
      .from("resources").select("storage_path,thumbnail_path").eq("id", data.id).maybeSingle();
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const paths = [row?.storage_path, row?.thumbnail_path].filter(Boolean) as string[];
    if (paths.length) await supabaseAdmin.storage.from("resource-library").remove(paths);
...
export const listComments = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: rows, error } = await context.supabase
      .from("resource_comments").select("*").eq("resource_id", data.resource_id).order("created_at", { ascending: true });
    if (error) throw new Error(error.message);
...
export const deleteComment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertCoachOrAdmin(context);
    const { error } = await context.supabase.from("resource_comments").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });