...
function PowerliftingRecordsView() {
  const [tab, setTab] = useState<"points"|"squat"|"bench"|"deadlift">("points");
  const { data = [], isPending } = useQuery({
    queryKey: ["powerlifting-rankings"],
    staleTime: 5 * 60_000,
    queryFn: async () => {
      const { data, error } = await (supabase as any).rpc("get_powerlifting_rankings");
      if (error) throw error;
      return (data ?? []) as any[];
    },
  });
  const value = (r:any) => tab === "points" ? Number(r.points ?? 0) : Number(r[tab + "_kg"] ?? 0);
  const sorted = [...data].filter((r:any)=>value(r)>0).sort((a:any,b:any)=>value(b)-value(a)).slice(0,10);
  return <div className="space-y-4">
    <SheetHeader className="text-left"><SheetTitle>JF Powerlifting Records</SheetTitle><SheetDescription>All-time JF athlete meet performances, including retired athletes.</SheetDescription></SheetHeader>
    <div className="grid grid-cols-4 gap-1 rounded-xl bg-muted p-1">{(["points","squat","bench","deadlift"] as const).map(t=><button key={t} type="button" onClick={()=>setTab(t)} className={cn("rounded-lg px-1 py-2 text-[11px] font-bold capitalize",tab===t?"bg-background shadow-sm":"text-muted-foreground")}>{t==="points"?"DOTS / GL":t}</button>)}</div>
    {isPending?<div className="py-8 text-center text-sm text-muted-foreground">Loading…</div>:<div className="overflow-hidden rounded-2xl border bg-card">{sorted.map((r:any,i)=><div key={r.id} className="flex items-center gap-3 border-b p-3 last:border-0"><div className="w-6 text-center text-sm font-black text-muted-foreground">{i+1}</div><div className="min-w-0 flex-1"><div className="truncate text-sm font-bold">{r.athlete_name}</div><div className="text-[10px] uppercase font-bold text-primary">{r.sex} · {r.competition_level||"competitor"}</div><div className="truncate text-[10px] text-muted-foreground">{r.meet_location||r.meet_name||"Meet"}{r.meet_date?` · ${new Date(r.meet_date+"T00:00:00").getFullYear()}`:""}</div></div><div className="text-right"><div className="text-sm font-black">{tab==="points"?`${Number(r.points).toFixed(2)} ${r.points_system}`:`${Number(r[tab+"_kg"])} kg`}</div><div className="text-[10px] text-muted-foreground">S {r.squat_kg} · B {r.bench_kg} · D {r.deadlift_kg}</div></div></div>)}</div>}
  </div>;
}
