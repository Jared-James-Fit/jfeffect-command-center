...
function useQuadrantStyles() {
  ...
  const save = (next: Record<TaskQuadrant, QuadStyle>) => {
    setStyles(next);
    try { localStorage.setItem(QUAD_STYLE_KEY, JSON.stringify(next)); } catch {}
  };
...
function TasksPage() {
  ...
  const { styles: quadStyles, update: updateQuadStyle, reset: resetQuadStyle } = useQuadrantStyles();
