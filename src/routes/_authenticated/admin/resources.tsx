...
function ResourceLibrary() {
  ...
  const { data: foldersData } = useQuery({ queryKey: ["rl-folders"], queryFn: () => foldersFn() });
  const folders: FolderRow[] = foldersData?.folders ?? [];

  const { data: resData, isLoading } = useQuery({
    queryKey: ["rl-resources", folderId, search],
  ...
  async function handleCreateFolder() {
    ...
    try {
      ...
      qc.invalidateQueries({ queryKey: ["rl-folders"] });
  ...
  async function handleDeleteFolder(id: string) {
    ...
    try {
      ...
      qc.invalidateQueries({ queryKey: ["rl-folders"] });
      qc.invalidateQueries({ queryKey: ["rl-resources"] });
  ...
  async function handleUpload(file: File) {
    ...
    try {
      ...
      const { error } = await supabase.storage.from("resource-library").uploadToSignedUrl(path, token, file);
...
function ResourceCard({ r, onOpen }: { r: Resource; onOpen: () => void }) {
  ...
  const { data: signed } = useQuery({
    queryKey: ["rl-thumb", r.id, r.storage_path, r.thumbnail_path],
