...
const empty = { athlete_id:"", client_id:"", athlete_name:"", sex:"male", bodyweight_kg:"", squat_kg:"", bench_kg:"", deadlift_kg:"", points:"", points_system:"DOTS", meet_name:"", meet_location:"", meet_date:"", competition_level:"local", notes:"" };
...
function AthleteRecordsAdmin(){
 ...
 const {data:athletes=[]}=useQuery({queryKey:["powerlifting-athletes-result-picker"],queryFn:async()=>{const {data,error}=await (supabase as any).from("powerlifting_athletes").select("id,client_id,athlete_name,sex,status").order("athlete_name");if(error)throw error;return data??[]}});
 ...
 <Field label="JF athlete"><Select value={form.athlete_id||"none"} onValueChange={v=>{const a=athletes.find((x:any)=>x.id===v);setForm((old:any)=>({...old,athlete_id:v==="none"?"":v,client_id:a?.client_id||"",athlete_name:a?.athlete_name||"",sex:a?.sex||"male"}))}}><SelectTrigger><SelectValue placeholder="Choose tracked athlete"/></SelectTrigger><SelectContent><SelectItem value="none">Choose athlete</SelectItem>{athletes.map((a:any)=><SelectItem key={a.id} value={a.id}>{a.athlete_name} · {a.status||"active"}</SelectItem>)}</SelectContent></Select></Field>
